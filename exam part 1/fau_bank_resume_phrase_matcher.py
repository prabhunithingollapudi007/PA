...
mypath=r'C:\Users\prabh\FAU\Study\SoSe2024\PA\Resume' #enter your path here where you saved the resumes

# Check if the directory exists
if not isdir(mypath):
    raise RuntimeError(f"Directory '{mypath}' does not exist")
...
def create_profile(resume):
    ...
    keyword_dict = pd.read_csv(r'C:\Users\prabh\FAU\Study\SoSe2024\PA\digital-transformation-manager-skills.csv',  encoding='ansi')
    ...
    technical_words = [nlp(text) for text in keyword_dict['Technical'].dropna(axis = 0)]
    leader_words = [nlp(text) for text in keyword_dict['Leadership'].dropna(axis = 0)]
    domain_words = [nlp(text) for text in keyword_dict['Domain Knowledge'].dropna(axis = 0)]
    soft_words = [nlp(text) for text in keyword_dict['Soft skills'].dropna(axis = 0)]

    matcher = PhraseMatcher(nlp.vocab)
    matcher.add('Technical', None, *technical_words)
    matcher.add('Leadership', None, *leader_words)
    matcher.add('Domain', None, *domain_words)
    matcher.add('Soft', None, *soft_words)
    doc = nlp(text)
